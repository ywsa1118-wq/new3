...
addBlock('s블록_참거짓', '%1', { color: '#0a0e83ff', outerline: '#10306aff', fontColor: '#ffffff' }, 
{ params: [{ type: 'Dropdown', options: [
    ['참', 'true'], 
    ['거짓', 'false']
], value: 'true', fontSize: 11 }], def: ['true'], map: { VALUE: 0 } }, 's블록', (sprite, script) => {
    return script.getField('VALUE', script) === 'true';
}, 'basic_boolean_field');
...
addBlock('s블록_수판단', '%1 이(가) %2 인가?', { color: '#000000', outerline: '#000000', fontColor: '#ffffff' }, 
{ params: [
    ...
    { type: 'Dropdown', options: [
        ...
        ['단위수', 'unit'],
        ['소수점이 있는 수', 'decimal']
    ], value: 'odd', fontSize: 11 }
], def: [{ type: 'number', params: ['1'] }, 'odd'], map: { VALUE: 0, TYPE: 1 } }, 's블록', (sprite, script) => {
    const value = Number(script.getValue('VALUE', script));
    const type = script.getField('TYPE', script);
...
addBlock('s블록_약수개수', '%1 의 약수 개수', { color: '#000000', outerline: '#000000', fontColor: '#ffffff' }, 
{ params: [{ type: 'Block', accept: 'string' }], def: [{ type: 'number', params: ['12'] }], map: { VALUE: 0 } }, 's블록', (sprite, script) => {
    const num = Math.abs(Math.floor(Number(script.getValue('VALUE', script))));
    if (num === 0) return 0;
    if (num === 1) return 1;
...
addBlock('s블록_두점거리', '%1 과 %2 의 거리', { color: '#000000', outerline: '#000000', fontColor: '#ffffff' }, 
{ params: [{ type: 'Block', accept: 'string' }, { type: 'Block', accept: 'string' }], def: [{ type: 'number', params: ['3'] }, { type: 'number', params: ['7'] }], map: { NUM1: 0, NUM2: 1 } }, 's블록', (sprite, script) => {
    const num1 = Number(script.getValue('NUM1', script));
    const num2 = Number(script.getValue('NUM2', script));
    
    return Math.abs(num2 - num1);
}, 'basic_string_field');

addBlock('s블록_진법변환', '%1 을(를) %2 에서 %3 로 반환한 값', { color: '#000000', outerline: '#000000', fontColor: '#ffffff' }, 
{ params: [
    { type: 'Block', accept: 'string' }, 
    { type: 'Dropdown', options: [
        ['원래', 'original'],
    ...
    { type: 'Dropdown', options: [
        ...
        ['유니코드', 'unicode'],
        ['아스키코드', 'ascii']
    ], value: 'binary', fontSize: 11 }
], def: [{ type: 'number', params: ['10'] }, 'original', 'binary'], map: { VALUE: 0, FROM: 1, TO: 2 } }, 's블록', (sprite, script) => {
    const value = script.getValue('VALUE', script);
    const from = script.getField('FROM', script);
    const to = script.getField('TO', script);
    
    // 같은 형식이면 그대로 반환
    if (from === to) return value;
    
    let intermediate = value; // 중간 값 (십진수)
    ...
    switch(from) {
        case 'original':
            ...
            break;
        case 'binary': // 이진법 → 십진수
            intermediate = parseInt(String(value), 2);
            if (isNaN(intermediate)) intermediate = 0;
            break;
        case 'decimal': // 십진법 (그대로)
            intermediate = Number(value);
            if (isNaN(intermediate)) intermediate = 0;
            break;
        case 'unicode': // 유니코드 → 문자
        case 'ascii': // 아스키코드 → 문자
            const codes = String(value).split(',').map(c => parseInt(c.trim()));
            intermediate = codes.map(code => String.fromCharCode(code)).join('');
            break;
    }
    ...
    switch(to) {
        case 'original':
            return intermediate;
        case 'binary': // 십진수 → 이진법
            const num = Number(intermediate);
            if (isNaN(num)) return '0';
            return num.toString(2);
        case 'decimal': // 십진법 (그대로)
            return Number(intermediate);
        case 'unicode': // 문자 → 유니코드
        case 'ascii': // 문자 → 아스키코드
            const text = String(intermediate);
...
addBlock('s블록_요일계산', '%1 년 %2 월 %3 일의 요일', { color: '#000000', outerline: '#000000', fontColor: '#ffffff' }, 
{ params: [
    { type: 'Block', accept: 'string' }, 
    { type: 'Block', accept: 'string' },
    { type: 'Block', accept: 'string' }
], def: [
    { type: 'number', params: ['2024'] }, 
    { type: 'number', params: ['1'] },
    { type: 'number', params: ['1'] }
], map: { YEAR: 0, MONTH: 1, DAY: 2 } }, 's블록', (sprite, script) => {
    const year = Number(script.getValue('YEAR', script));
    const month = Number(script.getValue('MONTH', script));
    const day = Number(script.getValue('DAY', script));
...
addBlock('s블록_간지계산', '%1 년의 간지', { color: '#000000', outerline: '#000000', fontColor: '#ffffff' }, 
{ params: [{ type: 'Block', accept: 'string' }], def: [{ type: 'number', params: ['2024'] }], map: { YEAR: 0 } }, 's블록', (sprite, script) => {
    const year = Number(script.getValue('YEAR', script));
    ...
    // 2015년이 을미년 (천간: 을=1, 지지: 미=7)
    const baseYear = 2015;
    const diff = year - baseYear;
    
    const stemIndex = (1 + diff) % 10;
    const branchIndex = (7 + diff) % 12;
...
addBlock('s블록_클립보드값', '현재 클립보드의 값', { color: '#0a0e83ff', outerline: '#10306aff', fontColor: '#ffffff' }, 
{ params: [], def: [], map: {} }, 's블록', async (sprite, script) => {
    try {
...
addBlock('s블록_iframe상태인가', '현재 iframe 상태인가?', { color: '#5DADE2', outerline: '#3498DB', fontColor: '#ffffff' }, 
{ params: [], def: [], map: {} }, 's블록', (sprite, script) => {
    const iframe = document.getElementById('s블록_iframe');
    return iframe !== null;
}, 'basic_boolean_field');
...
alert('s블록 0.6.0 업데이트 완료\n- 참/거짓 판단 블록이 추가되었습니다.\n- 복제본인가? 판단 블록이 추가되었습니다.\n- 현재 클립보드의 값 블록이 추가되었습니다.\n- 현재 iframe 상태인가? 판단 블록이 추가되었습니다.\n- (빈칸)이 [홀수/짝수/합성수/소수/단위수/소수점이 있는 수]인가? 판단 블록이 추가되었습니다.\n- (빈칸)의 약수개수 값 블록이 추가되었습니다.\n- (빈칸)과 (빈칸)의 거리 값 블록이 추가되었습니다.\n- (빈칸)을 [원래/이진법/십진법/유니코드/아스키코드]에서 [원래/이진법/십진법/유니코드/아스키코드]로 반환한 값 블록이 추가되었습니다.\n- (빈칸)년 (빈칸)월 (빈칸)일의 요일 값 블록이 추가되었습니다.\n- (빈칸)년의 간지 값 블록이 추가되었습니다.\n- 수학 블록들이 검정색으로 변경되고 맨 아래로 배치되었습니다.\n- NaN 값 블록에 -Infinity, null, undefined, -0 옵션이 추가되었습니다.\n- iframe하기/해제하기 블록이 추가되었습니다.\n- 콘솔 입력 블록에 로그/경고/에러 옵션이 추가되었습니다.\n- 선택창/프롬프트 답변 값 블록이 추가되었습니다.\n- localStorage 간편 저장/확인/삭제 블록이 추가되었습니다.');
